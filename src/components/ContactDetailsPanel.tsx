import { Check, Clock, Download, FileText, History, Image as ImageIcon, Link, Loader2, Mail, MessageSquare, Phone, RefreshCw, Save, Tag, Trash2, User, X } from 'lucide-react';
...
interface Props {
  ...
  canSyncWithWhatsApp?: boolean;
  isSyncingWithWhatsApp?: boolean;
  onSyncWithWhatsApp?: () => Promise<void>;
...
export const ContactDetailsPanel = ({ contact, notes, status, error, isSaving, isCreatingNote, isDarkMode, panelTitle = 'Dados do contato', profileName, profileAvatarUrl, canSyncWithWhatsApp = false, isSyncingWithWhatsApp = false, onSyncWithWhatsApp, initialTab = 'contact', onTabChange, conversation = null, accountId = null, onSetConversationCustomAttributes, conversationLabels = [], conversationAgents = [], conversationTeams = [], conversationParticipants = [], managementPendingAction = null, onSetConversationPriority, onAssignConversationAgent, onAssignConversationTeam, onSetConversationLabels, onSetConversationParticipants, onClose, onRetry, onUpdate, onCreateNote, contactConversations = [], contactConversationsStatus = 'idle', contactConversationsError = null, inboxes = [], onOpenConversation, onStartConversation, onNewConversation, onDelete, isDeleting = false, attachments = [], attachmentStatus = 'idle', attachmentError = null, hasMoreAttachments = false, onLoadMoreAttachments, onRetryAttachments, messages = [], onOpenImage }: Props) => {
  ...
  const contactDescription = typeof contact?.additionalAttributes.description === 'string' ? contact.additionalAttributes.description.trim() : '';
  const contactPreviews = attachments.slice(0, 4);
  ...
  return <><div className={`fixed inset-0 z-[80] flex h-[100dvh] w-screen flex-col overflow-hidden overscroll-contain ${surface} md:relative md:z-30 md:h-full md:w-[380px] md:max-w-[90vw] md:shrink-0 md:border-l`}>
      ...
      {status === 'ready' && contact && <>
        ...
        {tab === 'contact' && <>
          ...
          <section className={`rounded-xl border p-3 ${isDarkMode ? 'border-[#222d34] bg-[#182229]/60' : 'border-gray-200 bg-gray-50'}`}>
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 text-xs"><Field label="Telefone" icon={<Phone className="w-4 h-4" />} value={draft.phoneNumber} onChange={value => setDraft({ ...draft, phoneNumber: value })} inputClass={input} /><Field label="E-mail" icon={<Mail className="w-4 h-4" />} value={draft.email} onChange={value => setDraft({ ...draft, email: value })} inputClass={input} /><Field label="Identificador" icon={<Tag className="w-4 h-4" />} value={draft.identifier} onChange={value => setDraft({ ...draft, identifier: value })} inputClass={input} /><Field label="Empresa" icon={<User className="w-4 h-4" />} value={draft.companyName} onChange={value => setDraft({ ...draft, companyName: value })} inputClass={input} /></div>
          </section>
          <button type="button" onClick={() => selectTab('content')} className="w-full border-y border-white/10 py-4 text-left hover:bg-white/5"><span className="flex items-center gap-2 text-sm font-semibold"><ImageIcon className="h-5 w-5 text-[#8696a0]" />Mídia, links e docs<span className="ml-auto text-xs font-normal text-[#8696a0]">{attachments.length || ''}</span></span><span className="mt-3 grid grid-cols-4 gap-2">{contactPreviews.length ? contactPreviews.map(item => <span key={item.id} className="h-16 overflow-hidden rounded-lg bg-[#202c33]">{item.kind === 'image' ? <img src={item.thumbnailUrl || item.url} alt={item.title || 'Imagem'} className="h-full w-full object-cover" /> : item.kind === 'video' ? <video src={item.url} preload="metadata" className="h-full w-full object-cover" /> : <span className="flex h-full flex-col justify-between p-1.5"><FileText className="h-5 w-5 text-[#00a884]" /><span className="truncate text-[9px] text-[#8696a0]">{item.title || 'Documento'}</span></span>}</span>) : <span className="col-span-4 py-2 text-center text-xs font-normal text-[#8696a0]">Nenhuma mídia, link ou documento.</span>}</span></button>
          ...
          {onDelete && <button type="button" onClick={() => setIsDeleteConfirmationOpen(true)} className="flex w-full items-center justify-center gap-2 rounded-lg border border-red-500/30 px-3 py-2 text-xs font-semibold text-red-400 hover:bg-red-500/10"><Trash2 className="h-4 w-4" />Excluir contato</button>}
          {isDeleteConfirmationOpen && <div role="dialog" aria-label="Confirmar exclusão do contato" className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-xs"><p>Excluir este contato? Conversas e mensagens históricas não serão apagadas silenciosamente.</p><div className="mt-3 flex justify-end gap-2"><button type="button" disabled={isDeleting} onClick={() => setIsDeleteConfirmationOpen(false)}>Cancelar</button><button type="button" disabled={isDeleting} onClick={() => void onDelete?.().then(removed => { if (removed) onClose(); }).catch(() => setFeedback('Não foi possível excluir este contato porque ele ainda possui vínculos ou você não tem permissão.')).finally(() => setIsDeleteConfirmationOpen(false))} className="font-semibold text-red-400">{isDeleting ? 'Excluindo…' : 'Excluir contato'}</button></div></div>}
        </>}
    ...
    {status === 'ready' && contact && <div className="border-t border-white/10 p-3"><button type="button" disabled={!hasChanges || isSaving} onClick={() => setIsSaveConfirmationOpen(true)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#00a884] px-3 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Salvar alterações</button></div>}
  </div>{isSyncConfirmationOpen && <ConfirmDialog title="Sincronizar dados com o WhatsApp?" description="O nome e a foto atuais poderão ser substituídos pelos dados encontrados no WhatsApp." confirmLabel="Sincronizar" busyLabel="Sincronizando…" variant="primary" isBusy={isSyncingWithWhatsApp} onCancel={() => { if (!isSyncingWithWhatsApp) setIsSyncConfirmationOpen(false); }} onConfirm={() => void syncWithWhatsApp().finally(() => setIsSyncConfirmationOpen(false))} />}{isSaveConfirmationOpen && <ConfirmDialog title="Salvar alterações do contato?" description="As informações editadas serão atualizadas neste contato." confirmLabel="Salvar alterações" busyLabel="Salvando…" variant="primary" isBusy={isSaving} onCancel={() => { if (!isSaving) setIsSaveConfirmationOpen(false); }} onConfirm={() => void save().finally(() => setIsSaveConfirmationOpen(false))} />}</>;
};
