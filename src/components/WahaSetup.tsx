...
import { AlertCircle, CheckCircle2, ChevronDown, Loader2, LogOut, QrCode, RefreshCw, RotateCcw, Save, Users, X } from 'lucide-react';
...
export const WahaSetup = ({ accountId, inbox, isDarkMode, onSaved }: Props) => {
  ...
  useEffect(() => {
    ...
    void wahaClient.getCurrentHistoryImport(context)
      ...
      // A 404 merely means this inbox has not imported history yet.
      .catch(() => undefined);
    return () => { active = false; };
  }, [accountId, inbox.id]);
  useEffect(() => {
  ...
  const run = async (action: 'start' | 'restart' | 'logout' | 'qr') => {
  ...
  return <div className={`mx-auto max-w-3xl overflow-visible rounded-2xl border ${card}`}>
    ...
    {tab === 'whatsapp' && <section className="space-y-5">
      ...
      <div className="rounded-xl border border-white/10 p-4">
        <div className="mb-3 flex items-start gap-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#00a884] text-xs font-bold text-white">1</span><div><p className="text-xs font-bold">Escolha uma conexão existente</p><p className="mt-1 text-[11px] leading-4 text-[#8696a0]">Selecione o WhatsApp já criado para esta caixa. Use atualizar se uma conexão acabou de ser criada em outra tela.</p></div></div>
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]"><select value={selected} onChange={event => void selectSession(event.target.value)} className={`rounded-xl border px-3 py-3 text-sm ${isDarkMode ? 'border-[#2a3942] bg-[#111b21]' : 'border-gray-300 bg-white'}`}><option value="">Selecione um WhatsApp</option>{sessions.map(session => <option key={session.name} value={session.name}>{session.name} · {statusLabel[session.status] || session.status}</option>)}</select><button type="button" onClick={() => void refresh()} disabled={busy} className="rounded-xl border border-[#00a884]/40 px-3 text-xs font-bold text-[#00a884]"><RefreshCw className={`inline h-4 w-4 ${busy ? 'animate-spin' : ''}`} /> Atualizar lista</button></div>
      </div>
      ...
      <div className="rounded-xl border border-white/10 p-4">
        ...
        <div className="flex gap-2"><input value={newSession} onChange={event => setNewSession(event.target.value.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 80))} placeholder="Ex.: WhatsApp-Vendas" className={`min-w-0 flex-1 rounded-xl border px-3 py-3 text-sm ${isDarkMode ? 'border-[#2a3942] bg-[#111b21]' : 'border-gray-300 bg-white'}`} /><button type="button" onClick={() => void create()} disabled={!newSession || busy} className="rounded-xl bg-[#00a884] px-4 text-xs font-bold text-white disabled:opacity-40">Criar conexão</button></div>
      </div>
      ...
      <button type="button" onClick={() => void save()} disabled={!selected || busy || isAssociated} className="flex w-full justify-center gap-2 rounded-xl bg-[#00a884] py-3 text-xs font-bold text-white disabled:opacity-50">{busy && <Loader2 className="h-4 w-4 animate-spin" />}<CheckCircle2 className="h-4 w-4" />{isAssociated ? 'Esta conexão já está vinculada à caixa' : 'Usar esta conexão nesta caixa'}</button>
      {!isAssociated && selected && <p className="-mt-3 text-center text-[11px] text-[#8696a0]">Só clique depois de confirmar que este é o WhatsApp correto para esta caixa.</p>}
