...
const metaLabel = (inbox: Inbox) => inbox.reauthorizationRequired ? 'Reautorização necessária' : ({ connected: 'Conectado', connecting: 'Conectando', disconnected: 'Desconectado', error: 'Erro' }[String(inbox.additionalAttributes.meta_connection_status)] || 'Conectado');
...
export const HybridWhatsAppInboxConfig = ({ accountId, inbox, isDarkMode, onChanged }: Props) => {
  ...
  if (!configuration || !binding) return <div className={`rounded-xl border p-4 text-xs text-[#8696a0] ${card}`}>{error ? <p role="alert">{error}</p> : <><Loader2 className="mr-2 inline h-4 w-4 animate-spin" />Carregando modo híbrido…</>}</div>;
