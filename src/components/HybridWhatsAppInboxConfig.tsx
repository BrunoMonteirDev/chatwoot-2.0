...
export const HybridWhatsAppInboxConfig = ({ accountId, inbox, isDarkMode, onChanged }: Props) => {
  ...
  const refresh = useCallback(async () => {
    ...
    setConfiguration(nextConfiguration); setBinding(nextBinding); setSessions(nextSessions); setSession(nextBinding.wahaSession || '');
    setManaged(nextSessions.find(item => item.name === nextBinding.wahaSession) || null);
  }, [accountId, inbox.id]);
  ...
  const createAndConnect = async () => {
    if (busy) return;
    setBusy(true); setError(null);
    try { const created = await inboxService.createHybridWahaSession(accountId, inbox.id); setManaged(created); setSession(created.name); await refresh(); }
    catch (cause) { setError(errorMessageForUser(cause)); }
    finally { setBusy(false); }
  };

  const operate = async (operation: 'status' | 'start' | 'restart' | 'logout' | 'qr') => {
  ...
  useEffect(() => {
    const name = binding?.wahaSession || managed?.name;
    if (!name || managed?.connectionStatus === 'connected') return;
    const timer = window.setInterval(() => { void operate('status'); }, 3000);
    return () => window.clearInterval(timer);
  }, [binding?.wahaSession, managed?.name, managed?.connectionStatus]);
  ...
  return <section className={`space-y-4 rounded-xl border p-4 ${card}`} data-testid="hybrid-whatsapp-config">
    ...
    {configuration.hybridEnabled && <>
      <div className="rounded-lg border border-white/10 p-3 text-xs"><div className="flex items-center justify-between gap-2"><span className="font-semibold">Sessão WAHA</span><span className="text-[#8696a0]">{binding.wahaSession || 'Nenhuma sessão vinculada'} · {wahaLabel(binding.wahaStatus)}</span></div>{managed?.me?.id && <p className="mt-1 text-[#8696a0]">{managed.me.id}</p>}<div className="mt-3 flex flex-wrap gap-2">{!binding.wahaSession && <button type="button" onClick={() => void createAndConnect()} disabled={busy} className="rounded-lg bg-[#00a884] px-2 py-1.5 text-xs font-bold text-white">Criar e conectar WAHA</button>}{sessions.filter(item => item.name !== binding.wahaSession).length > 0 && <select aria-label="Vincular sessão existente" value={session} onChange={event => setSession(event.target.value)} disabled={busy} className="rounded-lg border border-[#2a3942] bg-transparent px-2 py-1.5 text-xs"><option value="">Vincular sessão existente</option>{sessions.filter(item => item.name !== binding.wahaSession).map(item => <option key={item.name} value={item.name}>{item.name}</option>)}</select>}{!binding.wahaSession && <button type="button" onClick={() => void bind()} disabled={busy || !session.trim()} className="inline-flex items-center gap-1 rounded-lg border border-[#00a884]/40 px-2 py-1.5 text-xs font-bold text-[#00a884]"><Link2 className="h-3.5 w-3.5" />Vincular</button>}{(binding.wahaSession || managed) && <><button type="button" onClick={() => void operate('qr')} disabled={busy || managed?.connectionStatus === 'connected'} className="rounded-lg border px-2 py-1.5"><QrCode className="inline h-3.5 w-3.5" /> Mostrar QR Code</button><button type="button" onClick={() => void operate('restart')} disabled={busy} className="rounded-lg border px-2 py-1.5"><RotateCcw className="inline h-3.5 w-3.5" /> Reconectar</button>{binding.wahaSession && <button type="button" onClick={() => void unbind()} disabled={busy} className="inline-flex items-center gap-1 rounded-lg border border-red-500/40 px-2 py-1.5 text-xs font-bold text-red-400"><Unlink className="h-3.5 w-3.5" />Desvincular</button>}<button type="button" onClick={() => setConfirmDelete(true)} disabled={busy} className="rounded-lg border border-red-500/40 px-2 py-1.5 text-red-400">Excluir sessão</button></>}</div>{qr && <div className="mt-3 rounded border p-3"><p>Abra o WhatsApp no celular e acesse Aparelhos conectados → Conectar aparelho.</p><img className="mt-2 h-48 w-48" alt="QR Code WAHA" src={qr.data.startsWith('data:') ? qr.data : `data:${qr.mimetype};base64,${qr.data}`} /></div>}{confirmDelete && <div role="dialog" className="mt-3 rounded border border-red-500/40 p-3"><p className="font-semibold">Excluir esta sessão WAHA?</p><p className="mt-1">Isso removerá a sessão do servidor WAHA. A conexão Meta não será alterada.</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => setConfirmDelete(false)}>Cancelar</button><button type="button" onClick={() => void deleteSession()} className="text-red-400">Excluir sessão</button></div></div>}</div>
