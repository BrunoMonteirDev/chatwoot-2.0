...
export const HybridWhatsAppInboxConfig = ({ accountId, inbox, isDarkMode, onChanged }: Props) => {
  ...
  useEffect(() => {
    const name = binding?.wahaSession || managed?.name;
    if (!name || managed?.connectionStatus === 'connected' || binding?.wahaStatus === 'missing') return;
    const poll = async () => {
      try {
  ...
  }, [accountId, binding?.wahaSession, binding?.wahaStatus, inbox.id, managed?.name, managed?.connectionStatus]);
