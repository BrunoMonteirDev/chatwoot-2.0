...
import { evolutionMetadataForInbox } from '../integrations/evolution/inbox';
  ...
  return <div className={`p-6 rounded-2xl border shadow-xl space-y-6 ${isDarkMode ? 'bg-[#111b21] border-[#222d34]' : 'bg-white border-[#d1d7db]'}`}>
    ...
    {!inboxCreationRoute && screen === 'list' && <>
      ...
      <div className="space-y-3">{inboxes.map(inbox => {
        ...
        return <div key={inbox.id} className={`p-4 rounded-xl border flex items-center justify-between ${card}`}><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[#00a884]/20 text-[#00a884] flex items-center justify-center"><Wifi className="w-5 h-5" /></div><div><h4 className="font-bold text-xs sm:text-sm">{inbox.name}</h4><p className="text-[11px] text-[#8696a0]">{native ? 'WhatsApp oficial' : configuration ? `WhatsApp · ${configuration.transports.map(transport => transport === 'meta_cloud' ? 'API oficial' : transport === 'waha' ? 'WhatsApp não oficial' : 'Conexão legada').join(' + ')}` : 'WhatsApp não oficial'}</p>{meta?.meta_onboarding_mode === 'coexistence' && <p className="text-[11px] text-[#8696a0]">WhatsApp Business App: {meta.meta_business_app_status === 'offboarded' ? 'reconexão necessária' : 'coexistência ativa'} · {historyLabel || 'Histórico aguardando'}</p>}{configuration && configuration.transports.length > 1 && <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-[#8696a0]">{configuration.transports.map(transport => <span key={transport}>{transport === 'meta_cloud' ? 'API oficial' : transport === 'waha' ? 'WhatsApp não oficial' : 'Conexão legada'}: <b className={transportStatuses[transport] === 'connected' ? 'text-[#00a884]' : transport === 'meta_cloud' ? 'text-red-400' : 'text-amber-500'}>{transportStatusLabel(transportStatuses[transport])}</b></span>)}</div>}</div></div><div className="flex items-center gap-2">{action}<button type="button" onClick={() => setInboxPendingDeletion(inbox)} aria-label={`Excluir ${inbox.name}`} title="Excluir caixa de entrada" className="rounded-xl border border-red-500/30 p-2 text-red-400 hover:bg-red-500/10"><Trash2 className="h-4 w-4" /></button></div></div>;
      })}</div>
    ...
    {inboxCreationRoute?.step === 'whatsapp' && !inboxCreationRoute.provider && <div className="mx-auto grid max-w-3xl gap-4 md:grid-cols-2"><button type="button" onClick={() => onNavigateInboxCreation?.({ step: 'whatsapp', provider: 'waha' })} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">WhatsApp não oficial</p><p className="mt-2 text-xs text-[#8696a0]">Cria uma inbox e conecta o celular por QR Code.</p></button><button type="button" onClick={() => onNavigateInboxCreation?.({ step: 'whatsapp', provider: 'meta' })} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">WhatsApp oficial</p><p className="mt-2 text-xs text-[#8696a0]">Cria uma inbox nativa pela Meta; o modo híbrido poderá ser ativado depois na mesma inbox.</p></button></div>}
    ...
    {!inboxCreationRoute && screen === 'waha' && accountId && selectedInbox && <WahaSetup accountId={accountId} inbox={selectedInbox} isDarkMode={isDarkMode} onSaved={onRefresh} />}
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <section className={`rounded-2xl border p-5 ${card}`}>
          ...
          <button type="button" className="mt-5 flex w-full items-start gap-4 rounded-xl border border-[#00a884] bg-[#00a884]/10 p-4 text-left ring-1 ring-[#00a884]/20">
            ...
            <span className="flex-1"><span className="flex items-center justify-between text-sm font-bold">WhatsApp <span className="rounded-full bg-[#00a884] px-2 py-0.5 text-[10px] text-[#0b141a]">Selecionado</span></span><span className="mt-1 block text-xs text-[#8696a0]">Conecte o celular por QR Code.</span></span>
          </button>
          <p className="mt-4 rounded-lg border border-[#2a3942] bg-black/10 p-3 text-[11px] leading-5 text-[#8696a0]">Outros canais podem ser criados no Chatwoot original. Conexões antigas continuam compatíveis.</p>
        </section>
        <section className={`rounded-2xl border p-5 ${card}`}>
    ...
    {inboxCreationRoute?.step === 'whatsapp' && inboxCreationRoute.provider === 'meta' && accountId && <MetaCloudSetup accountId={accountId} inbox={null} webhookUrl={chatwootWebhookUrl || ''} isDarkMode={isDarkMode} onSaved={async (saved) => { await onRefresh(); onNavigateInboxCreation?.({ step: 'agents', inboxId: String(saved.id) }); }} />}
