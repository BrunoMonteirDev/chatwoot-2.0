...
import { AlertCircle, CheckCircle2, ChevronLeft, Check, Loader2, MessageCircle, Plus, QrCode, RefreshCw, Unplug, Users, Wifi, WifiOff } from 'lucide-react';
...
type Screen = 'list' | 'create' | 'configure' | 'adopt';
...
export const EvolutionInboxesPanel: React.FC<Props> = ({ accountId, inboxes, inboxesStatus, inboxesError, onRefresh, isDarkMode }) => {
  ...
  const openConfig = (inbox: Inbox) => {
    ...
    if (accountId && chatwootWebhookUrl && inbox.webhookUrl !== chatwootWebhookUrl) {
      ...
      if (!metadata) return;
      void inboxService.saveEvolutionMetadata(accountId, inbox.id, metadata, chatwootWebhookUrl)
  ...
  const create = async () => {
    ...
    try {
      ...
      const instance = await evolutionService.createInstance(instanceNameFor(accountId, name));
      await evolutionService.configureWebhook(instance.instanceName);
      const saved = await inboxService.saveEvolutionMetadata(accountId, inbox.id, {
        evolution_provider: 'evolution', evolution_instance_name: instance.instanceName, evolution_instance_id: instance.instanceId,
      }, chatwootWebhookUrl);
      await onRefresh(); setSelectedInbox(saved); setScreen('configure'); setName('');
    } catch (cause) { setError(errorMessageForUser(cause)); }
  ...
  const configureExistingInbox = async () => {
    ...
    try {
      ...
      await evolutionService.configureWebhook(instance.instanceName);
      const saved = await inboxService.saveEvolutionMetadata(accountId, selectedInbox.id, {
        evolution_provider: 'evolution', evolution_instance_name: instance.instanceName, evolution_instance_id: instance.instanceId,
      }, chatwootWebhookUrl);
  ...
  return <div className={`p-6 rounded-2xl border shadow-xl space-y-6 ${isDarkMode ? 'bg-[#111b21] border-[#222d34]' : 'bg-white border-[#d1d7db]'}`}>
    <div className="flex items-center justify-between border-b pb-4 border-white/10">
      <div><h3 className="text-lg font-bold">Caixas de Entrada</h3><p className="text-xs text-[#8696a0]">WhatsApp via Evolution API (MVP).</p></div>
    ...
    {screen === 'list' && <>
      <div className="flex justify-end"><button type="button" disabled={!accountId} onClick={() => { setName(''); setError(null); setScreen('create'); }} className="px-3.5 py-2 bg-[#00a884] hover:bg-[#008069] disabled:opacity-40 text-white text-xs font-bold rounded-xl flex items-center gap-1.5"><Plus className="w-4 h-4" />Adicionar caixa de entrada</button></div>
      ...
      <div className="space-y-3">{inboxes.map(inbox => <div key={inbox.id} className={`p-4 rounded-xl border flex items-center justify-between ${card}`}><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[#00a884]/20 text-[#00a884] flex items-center justify-center"><Wifi className="w-5 h-5" /></div><div><h4 className="font-bold text-xs sm:text-sm">{inbox.name}</h4><p className="text-[11px] text-[#8696a0]">{isEvolutionInbox(inbox) ? `WhatsApp · Evolution API · ${instanceOf(inbox)}` : inbox.channelType}</p></div></div>{isEvolutionInbox(inbox) ? <button type="button" onClick={() => openConfig(inbox)} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Configurar</button> : inbox.channelType === 'Channel::Api' ? <button type="button" onClick={() => openEvolutionSetup(inbox)} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Configurar como Evolution</button> : <span className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-gray-500/15 text-[#8696a0]">Em breve</span>}</div>)}</div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <section className={`rounded-2xl border p-5 ${card}`}>
          ...
          <button type="button" className="mt-5 flex w-full items-start gap-4 rounded-xl border border-[#00a884] bg-[#00a884]/10 p-4 text-left ring-1 ring-[#00a884]/20">
            ...
            <span className="flex-1"><span className="flex items-center justify-between text-sm font-bold">WhatsApp <span className="rounded-full bg-[#00a884] px-2 py-0.5 text-[10px] text-[#0b141a]">Selecionado</span></span><span className="mt-1 block text-xs text-[#8696a0]">Conecte uma instância Evolution API por QR Code.</span></span>
          </button>
          <p className="mt-4 rounded-lg border border-[#2a3942] bg-black/10 p-3 text-[11px] leading-5 text-[#8696a0]">Outros canais podem ser criados no Chatwoot original. Nesta interface, o canal WhatsApp via Evolution é integrado de ponta a ponta.</p>
        </section>
        <section className={`rounded-2xl border p-5 ${card}`}>
          ...
          <button type="button" disabled={!name.trim() || creating || !chatwootWebhookUrl} onClick={() => void create()} className="mt-5 w-full py-3 bg-[#00a884] hover:bg-[#008069] text-[#0b141a] rounded-xl text-xs font-bold disabled:opacity-40 flex justify-center gap-2">{creating && <Loader2 className="w-4 h-4 animate-spin" />}{creating ? 'Criando caixa…' : 'Continuar para conectar'}</button>
        </section>
      </div>
    </div>}
