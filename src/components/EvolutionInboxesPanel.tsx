...
export const EvolutionInboxesPanel: React.FC<Props> = ({ accountId, inboxes, inboxesStatus, inboxesError, onRefresh, onInboxDeleted, isDarkMode, selectedInboxId = null, inboxCreationRoute = null, onOpenInbox, onNavigateInboxCreation, onCloseInbox }) => {
  ...
  return <div className={`p-6 rounded-2xl border shadow-xl space-y-6 ${isDarkMode ? 'bg-[#111b21] border-[#222d34]' : 'bg-white border-[#d1d7db]'}`}>
    ...
    {inboxCreationRoute?.step === 'whatsapp' && !inboxCreationRoute.provider && <div className="mx-auto grid max-w-3xl gap-4 md:grid-cols-2"><button type="button" onClick={() => onNavigateInboxCreation?.({ step: 'whatsapp', provider: 'waha' })} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">WhatsApp não oficial</p><p className="mt-2 text-xs text-[#8696a0]">Cria uma inbox e conecta uma sessão WAHA por QR Code.</p></button><button type="button" onClick={() => onNavigateInboxCreation?.({ step: 'whatsapp', provider: 'meta' })} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">WhatsApp oficial</p><p className="mt-2 text-xs text-[#8696a0]">Cria uma inbox nativa pela Meta; o modo híbrido poderá ser ativado depois na mesma inbox.</p></button></div>}
