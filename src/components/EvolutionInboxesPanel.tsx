  ...
  return <div className={`p-6 rounded-2xl border shadow-xl space-y-6 ${isDarkMode ? 'bg-[#111b21] border-[#222d34]' : 'bg-white border-[#d1d7db]'}`}>
    ...
    {screen === 'list' && <>
      ...
      <div className="space-y-3">{inboxes.map(inbox => {
        ...
        const nativeHistoryStatus = inbox.additionalAttributes.meta_history_status;
        const nativeHistoryProgress = inbox.additionalAttributes.meta_history_progress;
        const historyLabel = nativeHistoryStatus === 'completed' ? 'Histórico concluído' : nativeHistoryStatus === 'syncing' ? `Histórico importando${typeof nativeHistoryProgress === 'number' ? ` ${nativeHistoryProgress}%` : ''}` : nativeHistoryStatus === 'pending' ? 'Histórico aguardando' : nativeHistoryStatus === 'available' ? 'Histórico disponível' : nativeHistoryStatus === 'declined' ? 'Compartilhamento de histórico recusado' : nativeHistoryStatus === 'failed' ? 'Histórico indisponível' : meta?.meta_history_status === 'synced' ? 'Histórico sincronizado' : meta?.meta_history_status === 'importing' ? 'Histórico importando' : meta?.meta_history_status === 'ready' ? 'Histórico disponível' : meta?.meta_history_status === 'receiving' ? 'Histórico recebendo' : meta?.meta_history_status === 'waiting' ? 'Histórico aguardando autorização' : meta?.meta_history_status === 'failed' ? 'Histórico com falha' : meta?.meta_history_status === 'not_available' ? 'Histórico não autorizado' : null;
