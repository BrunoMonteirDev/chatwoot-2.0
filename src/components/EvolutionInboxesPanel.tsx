...
import { isNativeWhatsAppInbox, metaCloudMetadataForInbox, metaHistoryStatusForInbox, transportStatusesForInbox, whatsappConfigurationForInbox } from '../integrations/whatsapp/provider';
  ...
  return <div className={`p-6 rounded-2xl border shadow-xl space-y-6 ${isDarkMode ? 'bg-[#111b21] border-[#222d34]' : 'bg-white border-[#d1d7db]'}`}>
    ...
    {screen === 'list' && <>
      ...
      <div className="space-y-3">{inboxes.map(inbox => {
        ...
        const historyStatus = native ? metaHistoryStatusForInbox(inbox) : meta?.meta_history_status;
        const historyLabel = historyStatus === 'completed' || historyStatus === 'synced' ? 'Histórico concluído' : historyStatus === 'syncing' || historyStatus === 'importing' ? 'Histórico importando' : historyStatus === 'available' || historyStatus === 'ready' ? 'Histórico disponível' : historyStatus === 'pending' || historyStatus === 'waiting' ? 'Histórico aguardando' : historyStatus === 'declined' || historyStatus === 'not_available' ? 'Histórico recusado' : historyStatus === 'not_eligible' ? 'Histórico indisponível' : historyStatus === 'receiving' ? 'Histórico recebendo' : historyStatus === 'failed' ? 'Histórico com falha' : null;
