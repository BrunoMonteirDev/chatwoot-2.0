...
type Screen = 'list' | 'provider' | 'create' | 'configure' | 'adopt' | 'meta';
...
export const EvolutionInboxesPanel: React.FC<Props> = ({ accountId, inboxes, inboxesStatus, inboxesError, onRefresh, isDarkMode }) => {
  ...
  const create = async () => {
    ...
    try {
      ...
      const saved = await inboxService.saveWhatsAppTransport(accountId, inbox, 'evolution', {
  ...
  return <div className={`p-6 rounded-2xl border shadow-xl space-y-6 ${isDarkMode ? 'bg-[#111b21] border-[#222d34]' : 'bg-white border-[#d1d7db]'}`}>
    ...
    {screen === 'list' && <>
      ...
      <div className="space-y-3">{inboxes.map(inbox => {
        ...
        const action = configuration?.mode === 'hybrid' ? <button type="button" onClick={() => openConfig(inbox)} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Gerenciar</button> : isEvolutionInbox(inbox) ? <button type="button" onClick={() => openConfig(inbox)} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Configurar</button> : whatsappProviderForInbox(inbox) === 'meta_cloud' && meta?.meta_business_app_status === 'offboarded' ? <button type="button" onClick={() => { setSelectedInbox(inbox); setScreen('meta'); }} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Reconectar Meta</button> : whatsappProviderForInbox(inbox) === 'meta_cloud' ? <button type="button" onClick={() => openEvolutionSetup(inbox)} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Adicionar sessão Web</button> : inbox.channelType === 'Channel::Api' ? <button type="button" onClick={() => openEvolutionSetup(inbox)} className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#00a884]/15 text-[#00a884] border border-[#00a884]/30">Escolher transport</button> : <span className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-gray-500/15 text-[#8696a0]">Em breve</span>;
    ...
    {screen === 'provider' && <div className="mx-auto grid max-w-4xl gap-4 md:grid-cols-3"><button type="button" onClick={() => { setSelectedInbox(null); setScreen('meta'); }} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">API oficial Meta</p><p className="mt-2 text-xs text-[#8696a0]">Cadastro incorporado ou configuração manual.</p></button><button type="button" onClick={() => setScreen('create')} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">Sessão WhatsApp Web</p><p className="mt-2 text-xs text-[#8696a0]">Evolution via QR Code.</p></button><button type="button" onClick={() => { setSelectedInbox(null); setScreen('meta'); }} className={`rounded-2xl border p-5 text-left ${card}`}><p className="font-bold">Modo híbrido</p><p className="mt-2 text-xs text-[#8696a0]">Comece pela Meta; depois adicione a sessão complementar na mesma inbox.</p></button></div>}
