    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/65 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !isBusy) onCancel(); }}>
      <section role="alertdialog" aria-modal="true" aria-labelledby="confirm-dialog-title" aria-describedby="confirm-dialog-description" className="w-full max-w-md rounded-2xl border border-[#2a3942] bg-[#111b21] p-5 text-[#e9edef] shadow-2xl">
        <div className="flex items-start gap-3">
          <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${variant === 'danger' ? 'bg-red-500/15 text-red-400' : 'bg-[#00a884]/15 text-[#25d366]'}`}><AlertTriangle className="h-5 w-5" /></div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button type="button" onClick={onConfirm} disabled={isBusy} className={`rounded-lg px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${variant === 'danger' ? 'bg-red-500 hover:bg-red-600' : 'bg-[#00a884] hover:bg-[#008f73]'}`}>{isBusy ? (busyLabel || `${confirmLabel}…`) : confirmLabel}</button>
        </div>
      </section>
    </div>
  );
