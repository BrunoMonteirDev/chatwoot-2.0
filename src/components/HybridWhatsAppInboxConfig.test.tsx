...
describe('HybridWhatsAppInboxConfig', () => {
  it('opens the historical hybrid tab from the native Meta settings and synchronizes templates', async () => {
    await render({ hybridEnabled: false, outOfWindowStrategy: 'template', metaFailureStrategy: 'block' }, { wahaSession: null, wahaStatus: 'not_bound' });
    const { root, element } = roots[0];
    const sync = vi.spyOn(inboxService, 'syncWhatsAppTemplates').mockResolvedValue();
    await act(async () => { root.render(<MetaCloudSetup accountId={1} inbox={inbox} isDarkMode={false} onSaved={vi.fn()} />); });
    await act(async () => { (Array.from(element.querySelectorAll('button')).find(button => button.textContent === 'Sincronizar templates') as HTMLButtonElement).click(); });
    expect(sync).toHaveBeenCalledWith(1, 5);
    expect(element.textContent).toContain('Sincronização de templates iniciada com sucesso');
    await act(async () => { (Array.from(element.querySelectorAll('button')).find(button => button.textContent === 'WhatsApp Híbrido') as HTMLButtonElement).click(); });
    expect(element.querySelector('[data-testid="hybrid-whatsapp-config"]')).not.toBeNull();
    expect(element.textContent).toContain('WAHA: Não vinculado');
    expect(element.textContent).toContain('Colaboradores');
  });
