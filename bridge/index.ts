...
const loadGroupMetadata = async (transport: 'evolution' | 'waha', configuration: { evolutionInstanceName: string | null; wahaSessionName: string | null }, groupJid: string): Promise<GroupMetadata> => {
  if (transport === 'waha') {
    ...
    const avatarUrl = await wahaTransport.getChatAvatarUrl(configuration.wahaSessionName, groupJid).catch(() => undefined);
    return { ...metadata, ...(avatarUrl ? { avatarUrl } : {}), participants, transport, canEditDescription: true };
  }
...
app.get('/groups/metadata', async (request, response) => {
  ...
  try {
    ...
    const scopeKey = groupScopeKey(accountId, inboxId, groupJid);
    if (target.contactId) await identities.save([`group-contact:${scopeKey}`], String(target.contactId));
...
app.get('/contacts/profile', async (request, response) => {
  ...
  try {
    const inbox = await chatwootBridge.findWhatsAppInboxByIdForSession(accountId, inboxId, sessionHeaders);
    const transport = groupTransport(inbox.configuration, request.query.transport);
    if (!transport || transport === 'meta_cloud') throw new Error('A sincronização de perfil não é compatível com este transporte.');
    if (transport !== 'waha' || !inbox.configuration.wahaSessionName) throw new Error('A sincronização de perfil não é compatível com este transporte.');
    await wahaSessions.assertOwned(accountId, inboxId, inbox.configuration.wahaSessionName);
    const target = await chatwootBridge.conversationContactTargetForSession(accountId, conversationId, inboxId, sessionHeaders);
    ...
    const profile = await bestEffortContactProfile(
      () => chatwootBridge.withAccount(accountId, async () => {
      ...
      error => console.warn('[contacts] provider profile unavailable', { accountId, inboxId, conversationId, error: error instanceof Error ? error.message : 'unknown' }),
    );
    ...
    return response.status(502).json({ error: error instanceof Error ? error.message : 'Não foi possível carregar o perfil do contato.' });
  }
});
...
app.get('/config', (_request, response) => {
  if (!config.publicUrl) return response.status(503).json({ error: 'BRIDGE_PUBLIC_URL is not configured.' });
  try { return response.json({ bridgePublicUrl: config.publicUrl, chatwootWebhookUrl: chatwootWebhookUrl() }); }
  catch (error) { return response.status(503).json({ error: error instanceof Error ? error.message : 'Bridge callback URL is not configured.' }); }
});
...
const publicWahaConnection = (session: { status: string; connectionStatus: ConnectionStatus; me?: { id?: string; pushName?: string } }) => ({
...
const wahaQrForOwnedSession = async (sessionName: string) => {
  let session = await wahaTransport.getSession(sessionName);
  if (session.status !== 'WORKING') session = await wahaTransport.restartSession(sessionName);
...
app.get('/providers/waha/inboxes/:inboxId/connection', async (request, response) => {
  ...
  try {
    ...
    return response.json({ connection: owned ? publicWahaConnection(owned.session) : null });
  } catch (error) { return wahaOwnershipResponse(response, error) || wahaErrorResponse(response, error); }
});

app.post('/providers/waha/inboxes/:inboxId/connection', async (request, response) => {
  if (!(await enforceRateLimit(request, response, 'waha-session-configuration', 20, 60))) return;
  if (!(await requireBridgeAdministrator(request, response))) return;
  ...
  try {
    ...
    let created = false;
    if (!owned) {
      const sessionName = `waha_${randomUUID().replace(/-/g, '').slice(0, 24)}`;
      try { await wahaSessions.reserve({ ...context, sessionName, status: 'STARTING' }); }
      catch (error) {
        if (!(error instanceof WahaSessionOwnershipError && error.code === 'conflict')) throw error;
        owned = await ownedWahaSession(context);
    ...
    return response.status(created ? 201 : 200).json({ connection: publicWahaConnection(session), ...(qr ? { qr } : {}) });
  } catch (error) { return wahaOwnershipResponse(response, error) || wahaErrorResponse(response, error); }
});

app.post('/providers/waha/inboxes/:inboxId/connection/reconnect', async (request, response) => {
  if (!(await requireBridgeAdministrator(request, response))) return;
  const context = await wahaContext(request, response, request.body as Record<string, unknown>);
  if (!context || context.inboxId !== Number(request.params.inboxId)) return;
  try {
...
app.get('/providers/waha/inboxes/:inboxId/connection/qr', async (request, response) => {
  if (!(await requireBridgeAdministrator(request, response))) return;
  const context = await wahaContext(request, response, request.query as Record<string, unknown>);
  if (!context || context.inboxId !== Number(request.params.inboxId)) return;
  try {
...
app.delete('/providers/waha/inboxes/:inboxId/connection', async (request, response) => {
  if (!(await requireBridgeAdministrator(request, response))) return;
  const context = await wahaContext(request, response, request.body as Record<string, unknown>);
  if (!context || context.inboxId !== Number(request.params.inboxId)) return;
  try {
    const owned = await ownedWahaSession(context);
    if (owned) {
      try { await wahaTransport.deleteSession(owned.ownership.sessionName); }
      catch (error) { if (!(error instanceof WahaApiError && error.status === 404)) throw error; }
      await wahaSessions.remove(context.accountId, context.inboxId, owned.ownership.sessionName);
    }
    await clearWahaInbox(context.accountId, context.inboxId);
    return response.status(204).end();
  } catch (error) { return wahaOwnershipResponse(response, error) || wahaErrorResponse(response, error); }
...
const conversationForEvolutionIdentity = async (event: EvolutionIdentityEvent) => {
  ...
  if (event.chatType === 'group') {
    ...
    await chatwootBridge.saveEvolutionGroup(contact.id, event.remoteJid, event.name);
...
const assertActiveHybridSession = async (accountId: number, inboxId: number, sessionName: string) => {
  const ownership = await wahaSessions.assertOwned(accountId, inboxId, sessionName);
  if (ownership.status === 'cleanup_pending') throw new WahaSessionOwnershipError('conflict');
  return ownership;
};
...
app.post('/internal/official-whatsapp/waha/operations', async (request, response) => {
  ...
  try {
    await assertActiveHybridSession(accountId, inboxId, session);
...
const hybridBinding = async (request: express.Request, response: express.Response, action: 'bind' | 'unbind' | 'status') => {
  ...
  try {
    ...
    if (action === 'bind' || action === 'unbind') await assertActiveHybridSession(accountId, inboxId, sessionName);
    else {
      await assertActiveHybridSession(accountId, inboxId, sessionName);
      try {
        const session = await wahaTransport.getSession(sessionName);
        await wahaSessions.update(sessionName, { status: session.status, engine: session.engine, phone: session.me?.id });
...
const hybridSession = async (request: express.Request, response: express.Response, action: 'create' | 'status' | 'start' | 'restart' | 'logout' | 'qr' | 'delete' | 'list') => {
  ...
  try {
    if (action === 'list') {
      const owned = (await wahaSessions.list(accountId, inboxId)).filter(item => item.status !== 'cleanup_pending');
      const sessions = await Promise.all(owned.map(async ownership => {
...
app.post('/webhooks/waha', (request, response) => {
  ...
  void (async () => {
    ...
    if (groupLifecycle) {
      ...
      try {
        const ownership = await wahaSessions.get(groupLifecycle.session);
        if (ownership) {
  ...
  if (groupUpdates.length) {
    try {
      for (const update of groupUpdates) {
        ...
        groupMetadataCache.invalidate('evolution', update.groupJid);
